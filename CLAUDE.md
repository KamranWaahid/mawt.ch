# MAWT project instructions for Claude Code
...
Act as a careful senior React / Next.js frontend engineer, UI/UX designer, motion designer, and SEO-aware content editor.

Before editing, inspect the project carefully and preserve all approved parts of the design.

Do not blindly change code. Understand the intention first, then implement the cleanest targeted solution.
...
3. Identify what is already approved and should not be touched.
4. Explain the implementation plan briefly.
5. Then make focused edits.
...
4. How to test on desktop and mobile
5. Any risks or follow-up improvements

Do not rewrite entire components when a targeted edit is enough.
...
Use the existing dictionary pattern and pass the `dict` object through props.

Maintain both languages. If French copy is required and no exact translation is provided, create a polished, natural French equivalent.

Do not remove dictionary keys unless you verify they are unused.
...
* relevant URL helpers
* navigation data
* English and French dictionary entries

Localized URLs are rewritten by `proxy.ts` to shared English filesystem folders.
...
* all-caps typography unless required for a true acronym
